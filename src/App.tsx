...
function App() {
    ...
    return (
        /* Container */
        <div
            className="h-screen relative w-screen grid place-items-center content-center bg-cover bg-[url('https://free4kwallpapers.com/uploads/originals/2022/04/20/rubiks-cube-digital-art-wallpaper.jpg')]">
            {windows.map((elem, idx) => {
                return (
                    <DragElement key={elem.id}>
                        <section
                            className={`h-full w-full ease-linear shadow-2xl flex flex-col  rounded-lg cursor-auto `}>
                            {/* Header bar */}
                            <div
                                className={`bg-[#1a1f35] w-full min-h-[28px] rounded-t-lg  flex justify-end items-center space-x-1.5 overflow-hidden`}>
                  ...
                  <span className="hover:bg-[#8883] transition-colors w-12">
                    ...
                    >
                      <path d="M3 3v10h10V3H3zm9 9H4V4h8v8z"></path>
                                    ...
                                    <span className="hover:bg-[#f00c] transition-colors w-12"
                    ...
                    >
                      ...
                      ></path>
                            ...
                            <div
                                className=" bg-gray-700  w-full h-full border border-solid border-[#1a1f35] border-t-0  rounded-b-lg grid place-items-center  z-0 overflow-auto "
            ...
            <div
                className="fixed  w-full bottom-0 flex-none z-10 flex justify-center shadow-lg bg-white bg-opacity-75 border-t border-gray-300 border-opacity-20 backdrop-filter backdrop-blur-lg px-56 dark:bg-gray-800 dark:bg-opacity-75 dark:backdrop-filter dark:backdrop-blur-lg dark:border-gray-600 dark:border-opacity-20">
                {/*   Apps  */}
                <nav className="flex items-center justify-center space-x-1 h-14">
                    {appsIcon.map((app, index) => {
                        return (
                            ...
                            >
                                {isAppActive === app.id && <span
                                    className="absolute bottom-0 left-0 right-0 mx-4 h-1 rounded bg-blue-500"></span>}
...
const appsIcon: TAppsIcon[] = [
    ...
    },
    {
        ...
        content: () => <iframe className="w-full h-full" src="https://daisyui.com/components/range/"
